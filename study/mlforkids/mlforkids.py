...
class MLforKidsImageProject:
    ...
    def __init__(self, scratchkey: str):
        ...
        try:
            apiurl = self.__switchToTemporarySite(
                "https://machinelearningforkids.co.uk/api/scratch/" + scratchkey + "/train")
            with urllib.request.urlopen(apiurl) as url:
                self.__downloaded_training_images_list = json.loads(
                    url.read().decode())
        except urllib.error.HTTPError:
            raise RuntimeError(
                "Unable to retrieve machine learning project - please check that the key is correct")
    ...
    def __get_fname(self, trainingitem):
        extension = ".png" if trainingitem["imageurl"].lower().endswith(
            ".png") else ".jpg"
    ...
    def __get_training_images_generator(self):
        ...
        projectcachedir = str(os.path.expanduser(
            os.path.join(cachedir, cachelocation)))
            ...
            try:
                tf.keras.utils.get_file(origin=self.__switchToTemporarySite(trainingitem["imageurl"]),
                                        ...
                                        cache_subdir=os.path.join(
                                            cachelocation, trainingitem["label"]),
                ...
                print("ERROR: Unable to download training image from",
                      trainingitem["imageurl"])
                print(downloaderr)
                print("ERROR: Skipping training image and continuing without it",
                      trainingitem["imageurl"])
    ...
    def __train_model(self, trainingimagesdata):
        ...
        if trainingimagesdata.samples > 55:
            epochs = 15
        self.ml_model.fit(trainingimagesdata, epochs=epochs,
    ...
    def prediction(self, image_location: str):
        if hasattr(self, "ml_model") == False:
            raise RuntimeError(
                "Machine learning model has not been trained for this project")
        testimg = image.load_img(
            image_location, target_size=MLforKidsImageProject.IMAGESIZE)
